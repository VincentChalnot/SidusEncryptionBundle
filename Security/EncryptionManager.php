 ...
 * The cipher key is encrypted in the user with it's password (cleartext password) which means the cipher key can only be retrieved
 * at user's login. That's why it is passed along in the session (in clear form)
 * This way the cyperkey is never stored on the server (except in php's sessions) which means that the data are safe if the database or
 * the files are stolen.
 * For improved security you can lower PHP's session duration.
 * 
...
class EncryptionManager
{
     ...
     */
    public function __construct(Session $session, Registry $doctrine = null, Stopwatch $stopwatch = null, $autogenerateKey = false)
    {
    ...
    /**
     * Decrypt the cipher key used to encrypt/decrypt entreprise data using the user's password and saves it in the session
     *
    ...
    public function decryptCipherKey(UserEncryptionProviderInterface $user, $plainTextPassword)
    {
        ...
        if (!$user->getEncryptedCipherKey() && $this->autogenerateKey) {
            $this->createCipherKey($user, $plainTextPassword);
            $em = $this->doctrine->getManager();
            $em->persist($user);
            $em->flush();
        }
        $cipherKey = mcrypt_decrypt($this::$cipherKeyType, md5((string) $plainTextPassword), $user->getEncryptedCipherKey(), $this::$cipherKeyMode);
    ...
    public function encryptCipherKey(UserEncryptionProviderInterface $user, $plainTextPassword)
    {
        ...
        $user->setEncryptedCipherKey(mcrypt_encrypt($this::$cipherKeyType, md5((string) $plainTextPassword), $this->getCipherKey(), $this::$cipherKeyMode));
    }
    ...
    public function createCipherKey(UserEncryptionProviderInterface $user, $plainTextPassword)
    {
        ...
        $this->setCipherKey($cipherKey);
        $this->encryptCipherKey($user, $plainTextPassword, $cipherKey);
    }
    ...
    public function encryptString($string, $iv = null)
    {
        ...
        $encrypted = mcrypt_encrypt($this::$cipherDataType, $this->getCipherKey(), $string, $this::$cipherDataMode, $iv);
    ...
    public function decryptString($encryptedString, $iv = null)
    {
        ...
        $decrypted = mcrypt_decrypt($this::$cipherDataType, $this->getCipherKey(), $encryptedString, $this::$cipherDataMode, $iv);
        $this->stopWatch(__METHOD__);
    ...
    public function encryptStreamBlock($inputStream, $iv)
    {
        ...
        $encryptedBlock = mcrypt_encrypt($this::$cipherDataType, $this->getCipherKey(), $block, $this::$cipherDataMode, $iv);
    ...
    public function decryptStreamBlock($inputStream, $iv)
    {
        ...
        $block = mcrypt_decrypt($this::$cipherDataType, $this->getCipherKey(), $encryptedBlock, $this::$cipherDataMode, $iv);
        return $block;
    }
     ...
     * WARNING ! If you don't specify a fileSize parameter, your output file will be padded with \0 so it will break checksum
     * verifications or even the file itself depending of the format.
     * 
     * @param string $inputFilePath
     * @param string $outputFilePath
     * @param int $fileSize
     ...
     * @param string $category
     */
    protected function stopWatch($name, $category = null)
    {
        if ($this->stopwatch) {
